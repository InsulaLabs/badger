...
func LoadTLS(key, cert string, doGenOnFail bool) (*tls.Config, error) {
    ...
    if doGenOnFail {
      ...
      return nil, errors.New("empty key or cert path")
    }
	}
	c, err := tls.LoadX509KeyPair(cert, key)
	if err != nil {
    slog.Warn("failed to load cert and key from paths", "cert", cert, "key", key)
    if doGenOnFail {
      return GenerateTLS()
    }
		return nil, err
...
func GenerateTLS() (serverTLSConf *tls.Config, err error) {
	...
	ca := &x509.Certificate{
		...
		Subject: pkix.Name{
			...
			Province:      []string{""},
			Locality:      []string{"SSM"},
			StreetAddress: []string{"SBC"},
			PostalCode:    []string{"49783"},
		},
		NotBefore:             time.Now(),
